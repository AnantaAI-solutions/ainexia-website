        ...
        <div className="section-container relative">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {techCategories.map((category, index) => (
              <div
                key={index}
                className="group p-6 rounded-xl bg-secondary/20 border border-border hover:border-primary/40 transition-all duration-300 hover:shadow-[0_0_40px_-10px_hsl(199_89%_48%/0.3)]"
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {category.technologies.map((tech, i) => (
                    <span
                      key={i}
                      className="px-2.5 py-1 rounded-lg bg-background/50 text-xs text-foreground/80 border border-border/50"
                    >
                      {tech}
                    </span>
                  ))}
